...
class AI(Player):
    ...
    def compute_best_move(self, game_state):
        ...
        for move in moves:
            ...
            if piece_to_move:
                print(f"  [ROOT] Evaluating root move from square {move + 1}...")
                temp_game.move_piece(piece_to_move.pos, roll)
                score = self.expectiminimax(temp_game, self.depth - 1, False)
                print(f"  [ROOT] Move from square {move + 1} score: {score:.2f}")
                
                if score > best_score:
                    best_score = score
                    best_move = move
                    
        print(f"[ROOT] Computation complete. Total Visited Nodes: {self.visited_nodes}")
        print(f"[ROOT] Best Move: {best_move + 1} with Score: {best_score:.2f}\n")
        return best_move
    ...
    def expectiminimax(self, game_instance, depth, is_maximizing, is_chance_node=False, indent=""):
        ...
        if game_instance.is_game_over():
            ai_player = game_instance.players[game_instance.current_player_index]
            if len(self.pieces) == 0:
                print(f"{indent}[TERMINAL] AI Wins. Returned: +Infinity")
                return float('inf') 
            else:
                print(f"{indent}[TERMINAL] Human Wins. Returned: -Infinity")
                return float('-inf') 
                
        if depth == 0:
            eval_score = self.heuristic.evaluate(game_instance.get_game_state(None))
            print(f"{indent}[CUTOFF] Max depth reached. Heuristic Evaluation: {eval_score:.2f}")
            return eval_score
    ...
    def chance_node(self, game_instance, depth, is_maximizing, indent=""):
        ...
        print(f"{indent}[CHANCE / {node_type} NODE] Depth {depth}. Calculating expected value...")
